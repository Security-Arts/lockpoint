      ...
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
        <div className="flex items-center gap-3">
          ...
          <span className="hidden text-[11px] text-zinc-500 dark:text-zinc-400 sm:inline">
            Public commitments. Permanent record.
          </span>
        </div>
        ...
        <nav className="hidden items-center gap-1 sm:flex">
          ...
          {userId ? (
            <>
              <NavLink href="/me" label="My commitments" active={active.me} />
              <button
                type="button"
                onClick={() => onSignOut()}
                className="rounded-full px-3 py-2 text-xs font-semibold text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-300 dark:hover:bg-white/5 dark:hover:text-white"
      ...
      {open && (
        <div className="border-t border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-white/10 dark:bg-black sm:hidden">
          <div className="flex flex-col gap-2">
            <Link href="/" onClick={() => setOpen(false)} className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">Home</Link>
            {onGoRegistry ? (
              <button type="button" onClick={() => { setOpen(false); onGoRegistry(); }} className="text-left text-sm font-semibold text-zinc-800 dark:text-zinc-100">Registry</button>
            ) : (
              <Link href="/#registry" onClick={() => setOpen(false)} className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">Registry</Link>
            )}
            <Link href="/onboarding" onClick={() => setOpen(false)} className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">What is Lockpoint?</Link>
            <Link href="/roadmap" onClick={() => setOpen(false)} className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">Roadmap</Link>
            <Link href="/faq" onClick={() => setOpen(false)} className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">FAQ</Link>
            <div className="my-1 h-px bg-zinc-200 dark:bg-white/10" />
            <div className="flex flex-wrap gap-3 text-xs text-zinc-500 dark:text-zinc-400">
              <Link href="/terms" onClick={() => setOpen(false)} className="hover:text-zinc-900 dark:hover:text-white">Terms</Link>
              <Link href="/privacy" onClick={() => setOpen(false)} className="hover:text-zinc-900 dark:hover:text-white">Privacy</Link>
              <Link href="/disclaimer" onClick={() => setOpen(false)} className="hover:text-zinc-900 dark:hover:text-white">Disclaimer</Link>
            </div>
            {userId && (
              <button type="button" onClick={() => onSignOut()} className="mt-2 rounded-full border border-zinc-200 bg-white px-3 py-2 text-xs font-semibold text-zinc-800 dark:border-white/10 dark:bg-white/5 dark:text-zinc-100">
                Sign out
              </button>
            )}
          </div>
